...
{$IF CompilerVersion > 20.0}
   {$DEFINE CompilerVersion_21_UP}
{$ENDIF}

{$IF CompilerVersion > 30.0}
   {$DEFINE CompilerVersion_30_UP}
{$ENDIF}
...
function SaveStringToFile(AString:String;AFilePath:String{$IFDEF CompilerVersion_21_UP};AEncoding:TEncoding{$IFEND}):Boolean;
function GetStringFromFile(AFilePath:String{$IFDEF CompilerVersion_21_UP};AEncoding:TEncoding{$IFEND}):String;
{$IFDEF CompilerVersion_21_UP}
//比2007高的版本
...
function NewDelphiSubString(AStr:String;AIndex_StartWith0:Integer;ALength:Integer):String;
begin
  Result:={$IFDEF CompilerVersion_30_UP}AStr.Substring(AIndex_StartWith0,ALength){$ELSE}Copy(AStr,AIndex_StartWith0+1,ALength){$IFEND};
end;

function NewDelphiStringIndexOfName(ASubStr,AStr:String):Integer;
begin
  Result:={$IFDEF CompilerVersion_30_UP}AStr.IndexOf(ASubStr){$ELSE}Pos(ASubStr,AStr){$IFEND};
end;

function NewDelphiStringIndexOf(ASubStr,AStr:String):Boolean;
begin
  Result:={$IFDEF CompilerVersion_30_UP}AStr.IndexOf(ASubStr)<>-1{$ELSE}Pos(ASubStr,AStr)>0{$IFEND};
end;
...
begin
  ...
  try
    ...
    if w = TextFormatFlag[tfUnicode] then
      ...
      Result := TEncoding.{$IFDEF CompilerVersion_30_UP}ANSI{$ELSE}ASCII{$IFEND};
  finally
    Free;
  end;
...
begin
  Result:=False;
  AStringStream:=TStringStream.Create(AString{$IFDEF CompilerVersion_21_UP},AEncoding{$IFEND});
  try
...
begin
  ...
  begin
      ...
      AStringStream:=TStringStream.Create(''{$IFDEF CompilerVersion_21_UP},AEncoding{$IFEND});
      try
        {$IFDEF CompilerVersion_21_UP}
