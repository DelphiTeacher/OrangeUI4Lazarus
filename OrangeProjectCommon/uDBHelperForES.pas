...
type
  ...
  public
    ...
    //删除一条记录
    function DeleteRecord(ATableName:String;
                      AKeyFieldName:String;
                      AKeyFieldValue:Variant
                      ):Boolean;override;
  public
...
begin
          ...
          try
              ...
              try
                  ...
                  uBaseLog.HandleException(nil,'TDBHelperForESDB.Connect '
                                                ...
                                                +'SSL='+BoolToStr(ADataBaseConfig.FDBSSL)+' '
                                                +'Url='+ServerUrl+' '
                                                );
                  //
                  //  Self.FProxyType:=AIniFile.ReadString('','ProxyType','MYSQL');
                  //
                  ...
                  if ADataBaseConfig.FProxyHostName<>'' then
                  begin

                    //有些需要使用代理
    //                FNetHttpClient.ProxySettings:=TProxySettings.Create('www.orangeui.cn',808,'ggggcexx','138575wangneng',TURI.SCHEME_HTTP);
                    FNetHttpClient.ProxySettings:=TProxySettings.Create(ADataBaseConfig.FProxyHostName,StrToInt(ADataBaseConfig.FProxyHostPort),ADataBaseConfig.FProxyUserName,ADataBaseConfig.FProxyPassword,ADataBaseConfig.FProxyType);

                  end;

                  //连接数据库,ES
                  FNetHttpClient.Get(Self.ServerUrl+ADataBaseConfig.FDBDataBaseName,AResponseStream,FNetHTTPRequestHeaders);
    ...
    //              ShowException('数据库连接错误，请确认正确参数配置并重启服务端');
                  uBaseLog.HandleException(E,'TDBHelperForESDB.Connect '+E.Message);
                  uBaseLog.HandleException(E,'TDBHelperForESDB.Connect Response:'+AResponseStream.DataString);
                end;
              end;
...
//  FNetHttpClient.ResponseTimeout:=10*60*1000;//10分钟
...
function TDBHelperForESDB.DeleteRecord(ATableName, AKeyFieldName: String;
  AKeyFieldValue: Variant): Boolean;
var
  AUrl:String;
  AResponseStream:TStringStream;
begin
  ...
  try
        ...
        try
                ...
                //PUT和POST都能新增修改
                //FNetHttpClient.Put(AUrl,ARequestStream,AResponseStream,FNetHTTPRequestHeaders);
                FNetHttpClient.Delete(AUrl,AResponseStream,FNetHTTPRequestHeaders);

                //'{"_index":"user_center","_type":"tbluser","_id":"deepdigdb_1697278991594946560_38_20231208154607","_version":2,"result":"deleted","_shards":{"total":2,"successful":1,"failed":0},"_seq_no":54981150,"_primary_term":336}'
                FResponseJson:=SO(AResponseStream.DataString);
                uBaseLog.HandleException(nil,'TDBHelperForESDB.DeleteRecord '
                                            +' '+AUrl
                ...
                begin
                  ...
                  uBaseLog.HandleError(nil,'TDBHelperForESDB.DeleteRecord '
                ...
                if FResponseJson.S['result']<>'deleted' then
                begin
            ...
            begin
              ...
              //      DoLog(E,'SelfQuery');
              uBaseLog.HandleException(E,'TDBHelperForESDB.DeleteRecord '//+E.Message
                                          //上次使用时间,跟踪MYSQL是否在10分钟之内就断开连接了
                                          //+' '+FormatDateTime('YYYY-MM-DD HH:MM:SS:ZZZ',Self.FLastUseTime)
                                          +' '+AUrl);


              uBaseLog.HandleException(E,'TDBHelperForESDB.DeleteRecord Response:'
                                          +AResponseStream.DataString
                                          );
...
begin
  ...
  try
        ...
        try
                ...
                //PUT和POST都能新增修改
//                FNetHttpClient.Put(AUrl,ARequestStream,AResponseStream,FNetHTTPRequestHeaders);
                FNetHttpClient.Post(AUrl,ARequestStream,AResponseStream,FNetHTTPRequestHeaders);
...
//                                            +' '+AResponseStream.DataString);
            ...
            begin
              ...
              uBaseLog.HandleException(E,'TDBHelperForESDB.DoAddRecord Response:'
                                          +AResponseStream.DataString
                                          );
...
//                                            +' '+AUrl+' '+AQueryJsonStr+' '+AResponseStream.DataString);
          ...
          begin
            ...
            uBaseLog.HandleException(E,'TDBHelperForESDB.DoQueryBySearch Response:'
                                        +AResponseStream.DataString
                                        );
...
begin
  ...
  try
        ...
        try
                ...
                FNetHttpClient.Post(Self.ServerUrl+ATableName+'/'+'_sql',ARequestStream,AResponseStream,FNetHTTPRequestHeaders);
          ...
          begin
            ...
            uBaseLog.HandleException(E,'TDBHelperForESDB.DoQueryBySQL Response:'
                                        +AResponseStream.DataString
                                        );
...
begin
  ...
  try
        ...
        try
            ...
            AUrl:=Self.ServerUrl+ATableName+'/_mapping';
            if FDataBaseConfig.FDBVersion<>'5.0' then
            begin
              AUrl:=AUrl+'?include_type_name=true';
            end;
            ...
            //PUT和POST都能新增修改
            //FNetHttpClient.Put(AUrl,ARequestStream,AResponseStream,FNetHTTPRequestHeaders);
            FNetHttpClient.Get(AUrl,AResponseStream,FNetHTTPRequestHeaders);
            ...
            begin
              ...
              uBaseLog.HandleException(E,'TDBHelperForESDB.UpdateFieldList Response:'
                                          +AResponseStream.DataString
                                          );
...
//                ASuperObject.I['field_type']:=Ord(AFieldDef.DataType);
            ...
            begin
              ...
              FLastExceptMessage:=E.Message;
              //      DoLog(E,'SelfQuery');
              uBaseLog.HandleException(E,'TDBHelperForESDB.GetFieldList '//+E.Message
                                          //上次使用时间,跟踪MYSQL是否在10分钟之内就断开连接了
                                          //+' '+FormatDateTime('YYYY-MM-DD HH:MM:SS:ZZZ',Self.FLastUseTime)
                                          +' '+AUrl);


              uBaseLog.HandleException(E,'TDBHelperForESDB.GetFieldList Response:'
                                          +AResponseStream.DataString
                                          );
...
begin
  ...
  try
        ...
        try
                ...
                FResponseJson:=SO(AResponseStream.DataString);
                uBaseLog.HandleException(nil,'TDBHelperForESDB.GetRecord '
                                            +' '+AUrl
                                            +' '+AResponseStream.DataString);
                ...
                begin
                  FLastExceptMessage:=FResponseJson.AsJSON;
                  uBaseLog.HandleError(nil,'TDBHelperForESDB.GetRecord '
                                              +' '+AUrl+' '+AResponseStream.DataString);
                  Exit;
                end;
                //'{"_index":"searched_company_center","_type":"tblsearched_company","_id":"61529005A0CE441F90DC9768FB3E91DC","found":false,"_source":{}}'
            ...
            begin
              ...
              FLastExceptMessage:=E.Message;
              //      DoLog(E,'SelfQuery');
              uBaseLog.HandleException(E,'TDBHelperForESDB.GetRecord '//+E.Message
                                          //上次使用时间,跟踪MYSQL是否在10分钟之内就断开连接了
                                          //+' '+FormatDateTime('YYYY-MM-DD HH:MM:SS:ZZZ',Self.FLastUseTime)
                                          +' '+AUrl+' '+ARecordDataJson.AsJson);


              uBaseLog.HandleException(E,'TDBHelperForESDB.GetRecord Response:'
                                          +AResponseStream.DataString
                                          );
...
//                    ADataJson.S['es_url']:=AUrl;
                    ...
                    //PUT和POST都能新增修改
                    //FNetHttpClient.Put(AUrl,ARequestStream,AResponseStream,FNetHTTPRequestHeaders);
                    //在安德服务器上运行这句会报错,奇怪,但是AResponseStream又有内容的
                    FNetHttpClient.Post(AUrl,ARequestStream,AResponseStream,FNetHTTPRequestHeaders);
...
//                                                +' '+AResponseStream.DataString);
                    ...
                    begin
                        ...
                        begin
                          //要删除这个_scroll_id
                          AResponseStream.Size:=0;
                          FNetHttpClient.Delete(Self.ServerUrl+'_search/scroll'+'/'+FResponseJson.S['_scroll_id'],AResponseStream,FNetHTTPRequestHeaders);

                          //有些服务器可能禁用了删除这个功能
                          //XSuperJSON.TSuperParser.ParseJSON('<html>'#$D#$A'<head><title>403 Forbidden</title></head>'#$D#$A'<body>'#$D#$A'<center><h1>403 Forbidden</h1></center>'#$D#$A'<hr><center>openresty/1.19.3.1</center>'#$D#$A'</body>'#$D#$A'</html>'#$D#$A)
                          ATempJson:=SO(AResponseStream.DataString);
                          ADataJson.O['es_delete_scroll_response']:=ATempJson;
    ...
    //                  ARecordDataJson.S['_id']:=FResponseJson.O['hits'].A['hits'].O[I].S['_id'];
                      ...
                      ADataJson.A['ES_ID_List'].S[ADataJson.A['ES_ID_List'].Length]:=FResponseJson.O['hits'].A['hits'].O[I].S['_id'];
                    end;
                    ...
                    begin
                      ARequestStream.Position:=0;
                      AResponseStream.Size:=0;
                      FNetHttpClient.Delete(Self.ServerUrl+'_search/scroll'+'/'+AHTTPRequestParamJson.S['_scroll_id'],AResponseStream,FNetHTTPRequestHeaders);
            ...
            begin
              ...
              uBaseLog.HandleException(E,'TDBHelperForESDB.GetRecordList Response:'
                                          +AResponseStream.DataString
                                          );
...
//                  end;
//                end;
//                FNetHttpClient.Post(Self.ServerUrl+'_sql',ARequestStream,AResponseStream);
...
begin
  Result:=FDataBaseConfig.ServerUrl;
//  if not FDataBaseConfig.FDBSSL then
//  begin
//    Result:='http://'+FDataBaseConfig.FDBHostName+':'+FDataBaseConfig.FDBHostPort+'/';
//  end
//  else
//  begin
//   Result:='https://'+FDataBaseConfig.FDBHostName+':'+FDataBaseConfig.FDBHostPort+'/';
//  end;
end;
