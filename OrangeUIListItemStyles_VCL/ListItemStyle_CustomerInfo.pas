...
//  Self.lvTags.Height:=AlvTagsContentHeight;
  Self.lvTags.Height:=Ceil(Self.lvTags.Prop.CalcContentHeight);
  if Self.lvTags.Height=0 then
  begin
    //默认占一行的高度
    Self.lvTags.Height:=Ceil(Self.lvTags.Prop.ItemHeight);
  end;
...
begin
  ...
  lvTags.Height:=Ceil(lvTags.Prop.CalcContentHeight);
  Self.Height:=
    Self.pnlClient.Top
    +Self.lvTags.Top
    +Self.lvTags.Height+10;
