...
begin
  ...
  try
    ...
    D.Filter := TBitmapCodecManager.GetFilterString+'|'+'svg(*.svg)|*.svg';
    D.DefaultExt:='.png';
...
//    {$ENDIF}
    ADrawCanvas:=CreateDrawCanvas('TfrmDrawPicturePropertyEditor.PreviewPaint');
    try
      ADrawCanvas.Prepare(Canvas);

      if FDrawPicture.FSVGObject<>nil then
      begin
        ADrawRect:=RectF(0,0,Preview.Width,Preview.Height);
      end
      else
      begin
        ADrawRect:=RectF(0,0,FDrawPicture.Width,FDrawPicture.Height);
      end;
      GlobalDrawPictureParam.IsAutoFit:=True;
      ADrawCanvas.DrawPicture(GlobalDrawPictureParam,FDrawPicture,ADrawRect);
    finally
...
begin
  if FDrawPicture.FSVGObject<>nil then
  begin
    Preview.Width := 100;//FDrawPicture.CurrentPictureDrawWidth;
    Preview.Height := 100;//FDrawPicture.CurrentPictureDrawHeight;
  end
  else
  begin
