...
//      ('双击选择图片','DblClick To Select Pic','');
  ...
  // Facebook菜单
  Langs_AccountOperation : array[Low(TLangKind)..High(TLangKind)] of string =
      ('账号运营','Account Operation','');

  Langs_FBOperation : array[Low(TLangKind)..High(TLangKind)] of string =
      ('FB营销','FB Operation','');

  Langs_CustomerCollection : array[Low(TLangKind)..High(TLangKind)] of string =
      ('潜客采集','Customer Collection','');

  Langs_EffectTracking : array[Low(TLangKind)..High(TLangKind)] of string =
      ('效果追踪','Effect Tracking','');

  Langs_FBSetting : array[Low(TLangKind)..High(TLangKind)] of string =
      ('设置','FB Setting','');
