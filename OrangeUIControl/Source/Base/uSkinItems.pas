...
//    procedure InitSkinItemClass;virtual;
  ...
  public
    ...
    FOwnerProperties:TObject;
    constructor Create(AOwnerProperties:TObject;
...
//    procedure Add(AObject:TSkinItem);overload;
  end;
...
//procedure TSkinItems.Add(AObject: TSkinItem);
//begin
...
constructor TBaseSkinItems.Create(AOwnerProperties:TObject;const AObjectOwnership: TObjectOwnership;
...
function TRealSkinItem.GetDisplayName: string;
begin
  Result:=Caption;
end;
...
procedure TRealSkinItem.SetDisplayName(const Value: string);
begin
  Caption:=Value;
end;
...
begin
  ...
  begin
    Result:=TBaseSkinItems(Collection).GetListLayoutsManager;
  end;
end;
...
constructor TSkinItem.Create(ACollection:TCollection);
begin
  FColor:=TDrawColor.Create('Color','颜色');
  FColor.FAlpha:=0;
  FColor.FColor:=0;
...
constructor TBaseSkinTreeViewItem.Create(ACollection:TCollection);
begin
  inherited;
...
constructor TBaseSkinTreeViewItems.Create(AParent:TBaseSkinTreeViewItem;
                                      ...
                                      CreateObjectChange:Boolean=True);
begin
  Inherited Create(AOwnerProperties,AObjectOwnership,CreateObjectChange);
  FParent:=AParent;
end;
...
begin
  ...
  if (Self.FParent<>nil) then
  begin
    Result:=TBaseSkinTreeViewItems(Self.FParent.Collection);
    while (Result<>nil) and (Result.FParent<>nil) and (Result.FParent.Collection<>nil) do
    begin
      Result:=TBaseSkinTreeViewItems(Result.FParent.Collection);
    end;
  end;
end;
