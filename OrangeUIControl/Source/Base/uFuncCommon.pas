...
function NewDelphiSubString(AStr:String;AIndex_StartWith0:Integer;ALength:Integer):String;
begin
  Result:={$IFDEF CompilerVersion_30_UP}AStr.Substring(AIndex_StartWith0,ALength){$ELSE}Copy(AStr,AIndex_StartWith0+1,ALength){$ENDIF};
end;

function NewDelphiStringIndexOfName(ASubStr,AStr:String):Integer;
begin
  Result:={$IFDEF CompilerVersion_30_UP}AStr.IndexOf(ASubStr){$ELSE}Pos(ASubStr,AStr){$ENDIF};
end;

function NewDelphiStringIndexOf(ASubStr,AStr:String):Boolean;
begin
  Result:={$IFDEF CompilerVersion_30_UP}AStr.IndexOf(ASubStr)<>-1{$ELSE}Pos(ASubStr,AStr)>0{$ENDIF};
end;
...
begin
  ...
  try
    ...
    if w = TextFormatFlag[tfUnicode] then
      ...
      Result := TEncoding.{$IFDEF CompilerVersion_30_UP}ANSI{$ELSE}ASCII{$ENDIF};
  finally
    Free;
  end;
