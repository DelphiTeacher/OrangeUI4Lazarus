...
//    procedure InitSkinItemClass;override;
  ...
  TSkinChartType=(sctBar,sctLine,sctBezierLine,sctPie//,
                  //极坐标
                  //sctPolar
                  );
  //图表数据
...
begin
  ...
  begin
    ...
    ASeries:=TVirtualChartSeriesDataItems(ATooltipDataItem.Collection).FSeries;
...
begin
  ...
  begin
    ...
    if (ASeries.FChartType=sctBar)
      ...
      or (ASeries.FChartType=sctBezierLine)
      then
    begin
...
begin
  ...
  FDataItems:=TVirtualChartSeriesDataItems.Create(TBaseSkinItems(Collection).FOwnerProperties);
...
begin
  ...
  for I := 0 to Self.Count-1 do
  begin
    if (Items[I].FChartType=sctLine)
      or (Items[I].FChartType=sctBezierLine) then
    begin
      Items[I].FSameCharTypeIndex:=ASameCharTypeIndex;
      Inc(ASameCharTypeIndex);
    end;
...
begin
  ...
  if TVirtualChartSeriesDataItems(Self.Collection).FSeries.FDrawer=nil then
  begin
    Exit;
  end;
...
//
//        end;
        if IntersectRectF(ALastItemCaptionDrawRect,ADataItem.FCaptionRect) then
        begin
          Continue;
        end;
...
//  ABarSizePercent:Double;
  ...
  AOffset:Double;
  AStopPoint,AStartPoint:TPointF;
begin
  Inherited;
...
//      ADataItemPathRect.Bottom:=ADataItemRect.Bottom;
      ...
      begin
          ...
          AInnerPathActionItem:=TPathActionItem(ADataItem.FDrawPathActions.Add);
          AInnerPathActionItem.ActionType:=patAddArc;
          AInnerPathActionItem.X:=AOffset;
          AInnerPathActionItem.Y:=AOffset;
          AInnerPathActionItem.X1:=APathDrawRect.Width-AOffset;
          AInnerPathActionItem.Y1:=APathDrawRect.Height-AOffset;
          //要反方向画,不然填充不了扇形
...
//    APoints[I].Y:=APathDrawRect.Top+ADataItem.FLinePoint.Y;
    ...
    if I<Self.FSeries.FDataItems.Count-1 then
    begin
      AOldColor:=ASkinVirtualChartDefaultMaterial.FLineColorParam.Color.FColor;
...
//      FGraphics.DrawBeziers(AGPPen,ADrawPoints);
      ACanvas.DrawBezierLine(ASkinVirtualChartDefaultMaterial.FLineColorParam,
                             ADrawPoints
                              );
