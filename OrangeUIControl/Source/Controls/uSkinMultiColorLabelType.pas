...
function TColorTexts._AddRef: {$IFDEF LINUX}longint{$ELSE}Integer{$ENDIF};
begin
...
function TColorTexts._Release: {$IFDEF LINUX}longint{$ELSE}Integer{$ENDIF};
begin
  if FOwnerInterface <> nil then
...
function TColorTexts.QueryInterface({$IFDEF DELPHI}const{$ENDIF}{$IFDEF FPC}constref{$ENDIF} IID: TGUID;
