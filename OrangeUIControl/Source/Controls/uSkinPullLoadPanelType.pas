...
type
  ...
  public
    procedure DoOverRangePosValueChange(Sender:TObject;
                                        ...
                                        LastValue:Double;
                                        Step:Double;
                                        ContentMarginsTop,ContentMarginsBottom:Double;
                                        var NewValue:Double;
...
begin
  ...
  case Self.FSkinPullLoadPanelIntf.Prop.FLoadPanelType of
    ...
    begin
      //-2是为了避免挡往顶部分隔线
      Self.FSkinControlIntf.Top:=ControlSize(ContentMarginsTop)+ControlSize(PositionValue-Self.FSkinControlIntf.Height)-2;
    end;
    sborlptBottom:
    begin
      Self.FSkinControlIntf.Top:=ControlSize(TControl(Self.FSkinControlIntf.Parent).Height-ControlSize(ContentMarginsBottom)
                                              -PositionValue);
    end;
  end;
  //加载面板需要水平居中
  ...
  begin
    TSkinPullLoadPanelType(FSkinControlIntf.GetSkinControlType).
                            DoOverRangePosValueChange(Self,
                                                      ...
                                                      LastValue,
                                                      Step,
                                                      ContentMarginsTop,ContentMarginsBottom,
                                                      NewValue,
                                                      CanChange);
  end;
end;
